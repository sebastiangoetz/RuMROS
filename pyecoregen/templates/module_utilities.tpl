...
{%- macro generate_getterAttribute(a) %}
    def get{{a.name}}(self):
        return self.{{a.name}}
{%- endmacro %}
...
{%- macro generate_getterReference(o) %}
    def get{{o.name}}(self):
        {%- if o.upperBound != 1 %}
        if(self.{{o.name}} != None and self.{{o.name}} != []):
            names = []
            for item in self.{{o.name}}:
                names.append(item.getname())
            return names
        {% else %}
        if(self.{{o.name}} != None):
            return self.{{o.name}}.getname()
        {% endif -%}
        else:
            return None
{%- endmacro %}
...
{%- for a in c.eAttributes %}
    {{ generate_getterAttribute(a) -}}
{% endfor %}
{%- for r in c.eReferences %}
    {{ generate_getterReference(r) -}}
{% endfor %}
